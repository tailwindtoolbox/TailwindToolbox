...
document.onkeydown = function (evt) {
  ...
  if (
    ...
    srcTitle != document.activeElement &&
    srcDesc != document.activeElement &&
    srcUrl != document.activeElement &&
    srcAuthor != document.activeElement
  ) {
    evt.preventDefault();
    searchField.focus();
    togglePanel();
...
function updateSearchResults(value) {
  ...
  if (result.length === 0) {
    ...
    for (let item in result.slice(0, 4)) {
      ...
      let searchitem =
        '<span class="p-4 border-b flex justify-between items-center group hover:bg-teal-100"><a class="block flex-1 no-underline" href="' +
        result[item].url +
        '"><p class="font-bold text-sm text-indigo-600 hover:text-indigo-500"><span class="mr-2 text-teal-500">' +
        ...
        '</span></p><p class="hidden md:block text-xs text-teal-600">' +
        result[item].url +
        '</p><p class="text-sm py-1">' +
        result[item].description +
        '</p></a><a href="' +
        result[item].url +
        '"><img class="hidden md:block h-16 border-none" src="https://www.tailwindtoolbox.com/' +
        result[item].url_image +
        '"></a></span>';
